# relay-agent — GO implementation

Daemon client AnsibleRelay en Go. Maintient une connexion WSS persistante vers le relay server
...
Les packages agent sont dans le même module `relay-server` (défini dans `GO/go.mod`).
Les imports utilisent le préfixe `relay-server/cmd/agent/internal/...`.
...
### Même module que le serveur (`relay-server`)
...
# Build depuis GO/
cd GO/
go build -o bin/relay-agent ./cmd/agent

# Run (Linux)
RELAY_SERVER_URL=https://relay.example.com \
RELAY_WS_URL=wss://relay.example.com/ws/agent \
RELAY_PRIVATE_KEY=/etc/relay-agent/id_rsa \
RELAY_JWT_PATH=/etc/relay-agent/token.jwt \
./bin/relay-agent
```
...
| `RELAY_PRIVATE_KEY` | `/etc/relay-agent/id_rsa` | Chemin clef privée RSA |
| `RELAY_JWT_PATH` | `/etc/relay-agent/token.jwt` | Chemin JWT persisté |
| `RELAY_CA_BUNDLE` | `` (store système) | CA bundle custom |
| `RELAY_ASYNC_DIR` | `/var/lib/relay-agent/async` | Répertoire registre async |
