// relay-inventory — Binaire d'inventaire dynamique Ansible pour AnsibleRelay.
//
// Usage (Ansible external inventory script protocol) :
//
//	relay-inventory --list             Retourne tous les hôtes connectés au format JSON Ansible
//	relay-inventory --host <hostname>  Retourne les hostvars d'un hôte spécifique
//
...
func main() {
	...
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "Usage: relay-inventory --list | --host <hostname>\n")
		os.Exit(1)
	}
	...
	case "--host":
		if len(args) < 2 {
			fmt.Fprintf(os.Stderr, "Usage: relay-inventory --host <hostname>\n")
			os.Exit(1)
		}
	...
	default:
		fmt.Fprintf(os.Stderr, "Unknown flag: %s\nUsage: relay-inventory --list | --host <hostname>\n", args[0])
		os.Exit(1)
	}
}
